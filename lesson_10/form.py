...
class Form:
 ...
 @allure.step("Вводим имя: {name}")
    ...
    def surname(self, surname):
        self._driver.find_element(By.CSS_SELECTOR, "input[name='last-name']").clear()
        self._driver.find_element(By.CSS_SELECTOR, "input[name='last-name']").send_keys(surname)
    ...
    def phone_number(self, number):
        self._driver.find_element(By.CSS_SELECTOR, "input[name='phone']").clear()
        self._driver.find_element(By.CSS_SELECTOR, "input[name='phone']").send_keys(number)

    @allure.step("Вводим почтовый индекс: {code}")
    def zip_code(self, code):
        self._driver.find_element(By.CSS_SELECTOR, "input[name='zip-code']").clear()
        self._driver.find_element(By.CSS_SELECTOR, "input[name='zip-code']").send_keys(code)

    @allure.step("Вводим город: {city}")
    def city(self, city):
        self._driver.find_element(By.CSS_SELECTOR, "input[name='city']").clear()
        self._driver.find_element(By.CSS_SELECTOR, "input[name='city']").send_keys(city)

    @allure.step("Вводим страну: {country}")
    def country(self, country):
        self._driver.find_element(By.CSS_SELECTOR, "input[name='country']").clear()
        self._driver.find_element(By.CSS_SELECTOR, "input[name='country']").send_keys(country)

    @allure.step("Вводим должность: {job_position}")
    def job_position(self, job_position):
        self._driver.find_element(By.CSS_SELECTOR, "input[name='job-position']").clear()
        self._driver.find_element(By.CSS_SELECTOR, "input[name='job-position']").send_keys(job_position)

    @allure.step("Вводим название компании: {company}")
    def company(self, company):
        self._driver.find_element(By.CSS_SELECTOR, "input[name='company']").clear()
    ...
    @allure.step("Проверяем цвет опасного поля ZIP-кода")
    def danger_color(self):
        alert_danger_color = "rgba(248, 215, 218, 1)"
        color_zip = self._driver.find_element(By.ID, "zip-code").value_of_css_property("background-color")
        assert color_zip == alert_danger_color

    @allure.step("Проверяем цвет успешных полей")
    def success_color(self):
        alert_success_color = "rgba(209, 231, 221, 1)"
        fields = ["first-name", "last-name", "address", "e-mail", "phone", "city", "country", "job-position", "company"]
        for field in fields:
            color_field = self._driver.find_element(By.ID, field).value_of_css_property("background-color")
            assert color_field == alert_success_color